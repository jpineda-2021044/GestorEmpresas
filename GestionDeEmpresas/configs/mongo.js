...
export const connect = async()=>{
    try {
        ...
        mongoose.connection.on('disconnected', ()=> console.log('MongoDB | disconnected to mongodb'))
        mongoose.connection.on('reconnected', ()=> console.log('MongoDB | reconnected to mongodb'))
        return await mongoose.connect('mongodb://127.0.0.1:27017/GestionDeEmpresas')
    } catch (error) {
        console.error(error)
    }
}