...
export const login = async(req, res)=>{
    try {
        ...
        console.error(error)
        return res.status(504).send({message: 'Failed to login'})
    }
}
...
export const update = async(req, res)=>{
    try {
        ...
        if(!updateAdmin) return res.status({message: 'Updated admin', updateAdmin})
    } catch (error) {
        console.error(error)
...
export const deleteA = async(req, res)=>{
    try {
        let { id } = req.admin
        let admin = await Admin.finOneAndDelete({_id:id})
        if(!admin) return res.status(401).send({message: 'Admin not found and not deleted'})
